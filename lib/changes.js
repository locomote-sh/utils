...
/**
 * Convert an octal pair escape sequence in \xxx\xxx format to a string
 * containing its character equivalent.
 */
function octalToStr( octalPair ) {
    return Buffer.from([
        parseInt( octalPair.substr( 1, 3 ), 8 ),
        parseInt( octalPair.substr( 5, 3 ), 8 )
    ]).toString('utf8');
}
...
function correctExtendedChars( s ) {
    if( s.charCodeAt( 0 ) == 0x22 && s.charCodeAt( s.length - 1 ) == 0x22 ) {
        return s
            .slice( 1, -1 )
            .replace(/\\\d\d\d\\\d\d\d/g, octalToStr );
    }
    return s;
}
